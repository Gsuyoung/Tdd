...
class FeedServiceForPostFeedTest extends FeedServiceParentTest {
    ...
    void test2() throws Exception {
        ...
        given(myFileUtils.makeRandomFileName(mpf1)).willReturn(SAVED_PIC_NAME_1);
        //when(myFileUtils.makeRandomFileName(mpf1)).thenReturn(givenFilePath1);

        //void 메소드인 경우 given(임무부여) 방법, 주의사항 : when() 안에서 메소드 호출 금지
        doAnswer(invoctaion -> {
        ...
        assertAll(
                () -> {
                    ...
                    //myFileUtils.makeFolders메소드가 호출 되었고, 정확한 파라미터값("/feed/10")이 들어왔는지 단언
    ...
    void test3() throws Exception {
        ...
        assertAll(
                ...
                , () -> verify(feedPicMapper).insFeedPics(expectedFeedPicDto)
                , () -> {
                    for(int i=0; i<SAVED_FILE_NAMES.size(); i++) {
                        String picName = SAVED_FILE_NAMES.get(i);
